        ...
        match opcode {
            ...
            0b1100011 => {
                Ok(InstructionType::BType {
                    ...
                    rs2: Register::from_num(bitrange(combined,20,24))
                    .ok_or(ParseError::RegisterDecode(bitrange(combined,20,24)))?,
                    imm: bitrange(combined,8,11) + (bitrange(combined,25,30) << 5) + (bitrange(combined,7,7) << 11) + (bitrange(combined,31,31) << 12),
                    funct3: bitrange(combined,12,14) as u8,
                    opcode
                })
            },
            ...
            0b1101111 => {
                Ok(InstructionType::JType {
                    rd: Register::from_num(bitrange(combined,7,11))
                    .ok_or(ParseError::RegisterDecode(bitrange(combined,7,11)))?,
                    imm: bitrange(combined,21,30) + (bitrange(combined,20,20) << 11) + (bitrange(combined,12,19) << 12) + (bitrange(combined,31,31) << 20),
                    opcode
                })
            }
